...
BEGIN
SELECT [NAME] FROM [dbo].[BACKUP_FILTER];
END
`
	MIORRING_FILTER_SQL = `
USE [%s]
...
BEGIN
SELECT [NAME] FROM [dbo].[MIRRORING_FILTER];
END
`
	BACKUP_SETTING_SQL = `
USE [%s]
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[BACKUP_SETTING]') AND type in (N'U'))
BEGIN	
SELECT [APP],[FULL_BACKUP_PATH],[LOG_BACKUP_PATH],[KEEP_FULL_BACKUP_DAYS],[KEEP_LOG_BACKUP_DAYS] FROM [dbo].[BACKUP_SETTING]
END`
	SAVE_BACKUP_TRACE_SQL = `
USE [master]
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[BACKUP_TRACE]') AND type in (N'U'))
BEGIN
	IF EXISTS (SELECT * FROM Monitor.sys.objects WHERE object_id = OBJECT_ID(N'[Monitor].[dbo].[BACKUP_TRACE]') AND type in (N'U'))
