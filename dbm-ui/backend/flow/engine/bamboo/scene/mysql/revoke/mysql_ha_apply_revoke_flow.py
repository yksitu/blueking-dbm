...
from backend.flow.plugins.components.collections.mysql.revoke.check_tendb_ha_is_normal import (
    CheckTenDBHAIsNormalComponent,
)
...
class MySQLHAApplyRevokeFlow(RevokeFlowBase):
    """
    构建mysql主从部署对应的主机退回流程，单节点部署流程：MySQLHAApplyFlow().deploy_flow()
    触发时机：单据点击终止后
    ...
    def revoke_flow(self):
        ...
        for info in self.data["apply_infos"]:
            # 按照集群维度计算
            sub_flow_list.append(self.ha_apply_revoke_sub_flow(info))
        revoke_pipeline.add_parallel_sub_pipeline(sub_flow_list=sub_flow_list)
        revoke_pipeline.run_pipeline()

    def ha_apply_revoke_sub_flow(self, info: dict) -> SubProcess:
        """
        退回主机子流程,
        ...
        source_act = sub_pipeline.add_act(
            act_name=_("计算退回主机"),
            act_component_code=CheckTenDBHAIsNormalComponent.code,
            kwargs=asdict(
                CheckTenDBHAIsNormalComponent.kwargs(
                    check_proxy_hosts=info["proxy_ip_list"],
                    check_mysql_hosts=info["mysql_ip_list"],
                    start_proxy_port=self.data["start_proxy_port"],
        ...
        return sub_pipeline.build_sub_process(
            sub_name=_("集群[{}]计算回收主机ip".format([i["master"] for i in info["clusters"]]))
        )

    def clean_mysql_machine_sub_flow(
        self, domain_list: List[str], hosts: List[Dict], ports: List[int], bk_cloud_id: int
    ) -> SubProcess:
        """
        做回收主机的子流程
        1: 尝试清理机器和集群所有的元数据
            ...
            clean_db_meta_acts_list.append(
                {
                    ...
                    "kwargs": asdict(
                        DBMetaOPKwargs(
                            db_meta_class_func=MySQLDBMeta.mysql_ha_destroy_for_revoke.__name__,
